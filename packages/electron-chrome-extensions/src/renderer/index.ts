    ...
    const apiDefinitions: Partial<APIFactoryMap> = {
      ...
      storage: {
        factory: (base) => {
          ...
          const onChanged = (base && base.onChanged) as
            | EventWithDispatch<chrome.storage.StorageChangedEvent>
            | undefined
          if (onChanged) {
            ...
            onChanged.addListener = (callback: StorageCallback) => {
              ...
              if (!hasListeners) {
                sessionListener = (changes: StorageChangeRecord) =>
                  onChanged.dispatch(changes, 'session')
